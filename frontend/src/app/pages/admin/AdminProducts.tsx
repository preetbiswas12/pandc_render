...
export default function AdminProducts() {
  ...
  const handleProductTypeChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    setFormData(prev => {
      if (type === 'saree') {
        const sareeCategory = categories.find(c => c.name.toLowerCase() === 'saree' && c.isActive);
        return {
          ...prev,
          productType: type,
          unit: 'pieces',
          ...
          return (
             <div key={product._id} className="bg-white rounded-lg md:rounded-2xl overflow-hidden">
               ...
               <div className="p-2 sm:p-3 md:p-6">
                 ...
                 <p className="text-[10px] sm:text-xs md:text-sm opacity-70 mb-1 sm:mb-3 hidden sm:block">SKU: {product.sku}</p>
                 <div className="flex items-baseline gap-1 sm:gap-2 mb-1 sm:mb-3">
                   <span className="text-sm sm:text-lg md:text-2xl font-bold">₹{discountedPrice.toFixed(2)}</span>
                   {product.offerPercentage > 0 && (
                     <>
                       <span className="text-[10px] sm:text-xs md:text-sm opacity-50 line-through hidden sm:inline">₹{product.price}</span>
                       <span className="text-[10px] sm:text-xs md:text-sm text-green-600 font-medium hidden sm:inline">-{product.offerPercentage}%</span>
                     </>
                   )}
                 </div>
                 <p className="text-[10px] sm:text-xs md:text-sm opacity-70 mb-2 sm:mb-4 hidden sm:block">
                 ...
                 <div className="flex gap-1 sm:gap-2">
                   <button
                     ...
                     className="flex-1 border-2 border-blue-600 px-2 sm:px-4 py-1 sm:py-2 rounded-full font-medium hover:bg-blue-600 hover:text-white transition-all flex items-center justify-center gap-1 sm:gap-2 text-[10px] sm:text-sm"
                   >
                   ...
                   <button
                     ...
                     className="w-8 h-8 sm:w-10 sm:h-10 border-2 border-red-500 rounded-full flex items-center justify-center hover:bg-red-500 hover:text-white transition-all"
                   >
