...
import { Star, Trash2, Edit2, ThumbsUp, ThumbsDown, Loader2, User } from 'lucide-react';
import { 
  getProductRatings, 
  getUserRating, 
  createRating, 
...
export default function RatingComponent({
  ...
  const handleMarkHelpful = async (ratingId: string, isHelpful: boolean) => {
    const result = await markRatingHelpful(ratingId, isHelpful);
    if (result.success) {
      await fetchRatings();
    }
  };
          ...
          {ratings.map((rating) => (
            <div key={rating._id} className="border rounded-lg p-6">
              ...
              <div className="flex gap-4 text-sm">
                <button onClick={() => handleMarkHelpful(rating._id!, true)} className="flex items-center gap-1 text-gray-600 hover:text-[#0057c2] transition-colors">
                  <ThumbsUp size={14} />
                  <span>Helpful ({rating.helpful || 0})</span>
                </button>
                <button onClick={() => handleMarkHelpful(rating._id!, false)} className="flex items-center gap-1 text-gray-600 hover:text-red-500 transition-colors">
                  <ThumbsDown size={14} />
