                  ...
                  <div key={index} className="flex items-center gap-4 p-4 bg-gray-50 rounded-xl">
                    ...
                    <div className="text-right">
                      {item.offerPercentage > 0 ? (
                        <>
                          <p className="font-bold text-[#0057c2]">₹{(item.price * item.quantity).toFixed(2)}</p>
                          <p className="text-sm line-through opacity-50">₹{(item.originalPrice * item.quantity).toFixed(2)}</p>
                        </>
                      ) : (
