...
export default function AdminBanners() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editingBanner) {
      await updateBanner(editingBanner.id || editingBanner._id, bannerData);
    } else {
  ...
  const toggleActive = (banner: Banner) => {
    updateBanner(banner.id || banner._id, { isActive: !banner.isActive });
  };
  ...
  const moveOrder = (banner: Banner, direction: 'up' | 'down') => {
    ...
    if (swapBanner) {
      updateBanner(swapBanner.id || swapBanner._id, { order: banner.order });
    }
    updateBanner(banner.id || banner._id, { order: newOrder });
  };
    ...
    <div>
      ...
      <div className="space-y-4">
        {sortedBanners.map((banner) => (
          <div key={banner.id || banner._id} className="bg-white rounded-2xl overflow-hidden">
